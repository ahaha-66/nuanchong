...
export function App() {
  ...
  async function addPatient(values: { petName: string; species: string; birthDate?: string; sex?: string; consentDays: number | string }) {
    try {
      const pet = await api<{ id: string }>('/pets', { method: 'POST', body: { name: values.petName, species: values.species, birthDate: values.birthDate, sex: values.sex ?? 'UNKNOWN' } });
      const now = new Date();
      const days = typeof values.consentDays === 'string' ? Number(values.consentDays) : values.consentDays;
      const endsAt = new Date(now.getTime() + ((days && days > 0 ? days : 30) * 86400000));
      await api('/consents', { method: 'POST', body: { petId: pet.id, organizationId, scopes: ['CARE_PLAN', 'TASK_EXECUTIONS', 'OBSERVATIONS', 'TIMELINE'], startsAt: now.toISOString(), endsAt: endsAt.toISOString() } });
      message.success('患者已添加并授权');
      setAddPatientModal(false);
      await loadPatients();
      setSelectedId(pet.id);
    } catch (error) { message.error(error instanceof Error ? error.message : '添加患者失败'); }
  }
