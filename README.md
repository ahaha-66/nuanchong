...
本仓库是基于“养老/术后宠物连续照护协作”场景的 MVP 原型。目标是验证医院端与宠主端协同模式、数据隔离与审计策略、以及医院授权后数据共享的基本链路。当前实现适合本地演示和开发验证，不等同于已完成的生产系统。
...
- 宠主端
  - 微信小程序端首页“动态照护卡”展示
  - 今日照护任务、任务进度、关注事项与最近活动
  - 宠主可查看医院授权后的照护数据

- 平台基础
  - NestJS API + Prisma 数据模型
  - PostgreSQL RLS 运行时数据隔离
  - 审计与应急访问上下文管理
  - 共享契约包 `@nuanchong/contracts`
...
推荐使用根目录 `pnpm dev` 启动全量工程；也可分别启动单独应用：

```bash
pnpm --filter @nuanchong/api dev
pnpm --filter @nuanchong/hospital-web dev
pnpm --filter @nuanchong/miniapp dev
```

- 医院 Web 访问 `http://localhost:5173`，当前界面已包含患者详情摘要卡片，展示待办照护计划、任务完成与最新事件。
- 小程序可在微信开发者工具中打开 `apps/miniapp`，当前首页已实现“动态照护卡”格式，展示照护任务、关注项和最近活动。
